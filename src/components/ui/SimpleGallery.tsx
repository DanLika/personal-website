...
const SimpleGallery: React.FC<SimpleGalleryProps> = ({ images, title }) => {
    ...
    return (
        <>
            ...
            {/* Lightbox - rendered via Portal to ensure it's above everything */}
            {createPortal(
                <AnimatePresence>
                    {selectedImageIndex !== null && (
                        ...
                        >
                            ...
                            <button
                                onClick={() => setSelectedImageIndex(null)}
                                className="absolute top-4 right-4 md:top-8 md:right-8 text-white/70 hover:text-white p-2 rounded-full bg-white/10 hover:bg-white/20 transition-all z-[10000]"
                            >
                                <X size={24} />
                            </button>
                            ...
                            <button
                                onClick={handlePrev}
                                className="absolute left-4 md:left-8 text-white/70 hover:text-white p-3 rounded-full bg-white/10 hover:bg-white/20 transition-all z-[10000]"
                            >
                                <ChevronLeft size={32} />
                            </button>
                            ...
                            <button
                                onClick={handleNext}
                                className="absolute right-4 md:right-8 text-white/70 hover:text-white p-3 rounded-full bg-white/10 hover:bg-white/20 transition-all z-[10000]"
                            >
